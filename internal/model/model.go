...
func OrientOutward(source Model) Model {
	if len(source.Faces) == 0 || len(source.Verts) == 0 {
		return Prepare(source)
	}
	center := math3d.Vec3{}
	for _, vertex := range source.Verts {
	...
	for index, face := range source.Faces {
		...
		for _, vertex := range vertices {
			faceCenter = faceCenter.Add(source.Verts[vertex])
		}
...
func compileTopology(verts []math3d.Vec3, authored []Edge, faces []Face) *Topology {
	...
	if len(verts) > 0 {
		...
		for _, vertex := range verts {
			...
			if distance > topology.BoundsRadius {
				topology.BoundsRadius = distance
			}
		}
	}
...
func faceNormal(verts []math3d.Vec3, indices []int) (math3d.Vec3, float64) {
	...
	for i, current := range indices {
		...
		if current < 0 || current >= len(verts) || next < 0 || next >= len(verts) {
			return math3d.Vec3{}, 0
		}
...
func (m Model) Validate() error {
	...
	for faceIndex, face := range m.Faces {
		...
		for _, vertex := range face.Vertices {
			if unique[vertex] {
				return fmt.Errorf("face %d: repeated vertex %d", faceIndex, vertex)
			}
			unique[vertex] = true
		}
		if normal, _ := faceNormal(m.Verts, face.Vertices); normal.Length() <= 1e-9 {
			return fmt.Errorf("face %d: degenerate area", faceIndex)
		}
	}
	return nil
}
